...
`n_number` is canonical (leading `N`) from `AC.n_number`. Blank fleet/portable → NULL. Do not guess from `call_sign`. `contact_*` columns stay NULL in v1 (this dump has zero CL rows). These are **labels**, not a name-match score.
